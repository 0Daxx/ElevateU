...
export default function RoadmapsScreen({ navigation }: RoadmapsScreenProps) {
  ...
  const handleOpenRoadmap = (roadmap: RoadmapItem) => {
    if (navigation) {
      // React Navigation usage:
      navigation.navigate('RoadmapDetail', { id: roadmap.id, title: roadmap.title });
    } else {
      // Expo Router usage:
      // router.push({ pathname: '/roadmap/[id]', params: { id: roadmap.id } });
      console.log(`Opening Roadmap: ${roadmap.title}`);
    }
  };
      ...
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.card, { backgroundColor: theme.cardBackground, borderColor: theme.cardBorder }]}>
          ...
          <Text style={[styles.heroDescription, { color: theme.textSecondary }]}>
            Initial screen shows several mock roadmaps like roadmap.sh. Tapping any card opens a detailed roadmap view.
          </Text>
        </View>
        ...
        <View style={styles.gridContainer}>
          {ROADMAPS_DATA.map((item) => (
            <TouchableOpacity
              key={item.id}
              activeOpacity={0.7}
              onPress={() => handleOpenRoadmap(item)}
              style={[
                styles.gridCard,
            ...
            >
              ...
              <Text
                style={[styles.roadmapDescription, { color: theme.textSecondary }]}
                numberOfLines={3}
              >
                {item.description}
              </Text>
              ...
              <View style={styles.cardFooter}>
                ...
                <View style={[styles.openButton, { backgroundColor: theme.badgeBackground }]}>
                  <Text style={[styles.openButtonText, { color: theme.textPrimary }]}>
                    Open
                  </Text>
                </View>
              </View>
        ...
        <View style={[styles.card, { backgroundColor: theme.cardBackground, borderColor: theme.cardBorder }]}>
          <View style={styles.cardHeaderRow}>
            ...
            <View style={[styles.badge, { backgroundColor: theme.badgeBackground }]}>
              <Text style={[styles.badgeText, { color: theme.badgeText }]}>Optional</Text>
            </View>
          </View>
              ...
              return (
                <TouchableOpacity
                  ...
                  style={[
                    styles.chip,
                    {
                      backgroundColor: isActive ? theme.primary : theme.badgeBackground,
